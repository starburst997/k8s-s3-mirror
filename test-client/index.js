...
async function uploadFile(bucket, key, filePath) {
  try {
    console.log(chalk.blue(`📤 Uploading ${chalk.bold(filePath)} to ${chalk.bold(`${bucket}/${key}`)}...`));
...
async function downloadFile(bucket, key, outputPath) {
  try {
    console.log(chalk.blue(`📥 Downloading ${chalk.bold(`${bucket}/${key}`)} to ${chalk.bold(outputPath)}...`));
    ...
    console.log(chalk.green(`✅ Successfully downloaded to ${chalk.bold(outputPath)}`));
...
async function listFiles(bucket, prefix = '') {
  try {
    console.log(chalk.blue(`📋 Listing files in ${chalk.bold(bucket)}${prefix ? ` with prefix ${chalk.bold(prefix)}` : ''}...`));
